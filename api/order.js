// Принимает заявку с сайта и отправляет её ботом в чат + лидом в amoCRM.
// Токены и id берутся из переменных окружения Vercel — в коде их нет.

import { sendToAmo } from './amocrm.js';

const MAX = 500;

function clean(v) {
  return String(v == null ? '' : v).replace(/\s+/g, ' ').trim().slice(0, MAX);
}

// экранируем данные от пользователя, чтобы не поломать HTML-разметку
function esc(v) {
  return clean(v).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
}

export default async function handler(req, res) {
  if (req.method === 'OPTIONS') return res.status(204).end();
  if (req.method !== 'POST') return res.status(405).json({ ok: false, error: 'method' });

  const TOKEN = process.env.BOT_TOKEN;
  const CHAT_ID = process.env.CHAT_ID;
  if (!TOKEN || !CHAT_ID) {
    return res.status(500).json({ ok: false, error: 'Не заданы BOT_TOKEN или CHAT_ID' });
  }

  let body = req.body;
  if (typeof body === 'string') {
    try { body = JSON.parse(body); } catch { body = {}; }
  }
  body = body || {};

  const name = esc(body.name);
  const phone = esc(body.phone);
  const telegram = esc(body.telegram);
  const city = esc(body.city);
  const comment = esc(body.comment);

  const items = Array.isArray(body.cart) ? body.cart.slice(0, 30) : [];

  if (!name || !phone) return res.status(400).json({ ok: false, error: 'Не заполнены имя или телефон' });
  if (!items.length) return res.status(400).json({ ok: false, error: 'Пустая корзина' });

  const lines = items.map((i) => {
    const qty = Math.max(1, Math.min(99, parseInt(i && i.qty, 10) || 1));
    return `${esc(i && i.name)} - ${esc(i && i.color).toLowerCase()} - ${qty} шт`;
  });

  const totalNum = Number(body.total);
  const total = Number.isFinite(totalNum) && totalNum > 0 ? Math.round(totalNum) : null;

  const text = [
    '<b>НОВЫЙ ЗАКАЗ</b>',
    '',
    `Имя: ${name}`,
    `Номер телефона: ${phone}`,
    `Telegram: ${telegram}`,
    `Город: ${city}`,
    `Комментарий: ${comment}`,
    '',
    '<b>ЗАКАЗАЛ(А):</b>',
    ...lines,
    ...(total ? ['', `Сумма: ${total} BYN`] : []),
    '',
    'ЗАКАЗ ЗАКРЫТ?'
  ].join('\n');

  const sendTelegram = async () => {
    const tg = await fetch(`https://api.telegram.org/bot${TOKEN}/sendMessage`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        chat_id: CHAT_ID,
        text,
        parse_mode: 'HTML',
        disable_web_page_preview: true,
        reply_markup: {
          inline_keyboard: [[
            { text: 'Да ✅', callback_data: 'close_yes' },
            { text: 'Нет ❌', callback_data: 'close_no' }
          ]]
        }
      })
    });
    const data = await tg.json();
    if (!data.ok) throw new Error('Telegram отклонил сообщение: ' + JSON.stringify(data));
  };

  const [tgResult, amoResult] = await Promise.allSettled([
    sendTelegram(),
    sendToAmo({ name, phone, telegram, city, comment, lines, total })
  ]);

  if (amoResult.status === 'rejected') {
    console.error('amoCRM error:', amoResult.reason);
  }

  if (tgResult.status === 'rejected') {
    console.error('Telegram error:', tgResult.reason);
    return res.status(502).json({ ok: false, error: 'Не удалось связаться с Telegram' });
  }

  return res.status(200).json({ ok: true });
}
